...
// generated-by-copilot: Ensure user1 exists in test data with valid favorites
beforeAll(() => {
  ...
  if (!users.find(u => u.username === 'user1')) {
    users.push({
      username: 'user1',
      password: 'testpass',
      favorites: books.length > 0 ? [String(books[0].id)] : [],
      readingList: {}
    });
...
describe('Favorites API', () => {
  ...
  it('POST /api/favorites should not duplicate favorites', async () => {
    ...
    // generated-by-copilot: Route returns 409 for duplicate favorites
